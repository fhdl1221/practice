...
for (let i = 0; i < numbers4.length; i++) {
  if (numbers4[i] % 2 === 0) newNumbers2.push(numbers4[i]);
}
...
for (let index = 0; index < memoList.length; index++) {
  if (memoList[index].isDone === false) console.log(memoList[index]);
}
...
for (let i = 0; i < userList.length; i++) {
  if (userList[i].age % 2 === 0) console.log(userList[i].name);
}
