...
async function getRecipe() {
  const res = await axios.get("https://dummyjson.com/recipes/meal-type/lunch");
  const data = res["data"];
  console.log(data);
}
//getRecipe();
...
async function getTodo(n) {
  const res = await axios.get(`https://dummyjson.com/todos/user/${n}`);
  const data = res["data"];
...
async function getComments() {
  ...
  const result = comments.map((info) => ({
    body: info.body,
    likes: info.likes,
