...
const BASE_URL = `https://www.dummyjson.com`;
...
async function getProductReview(n) {
...
async function getProductRating(n) {
  const res = await axios.get(`${BASE_URL}/products/${n}`);
  const data = res["data"];
...
async function getPostTagList(tag) {
  const res = await axios.get(`${BASE_URL}/posts/tag/${tag}`);
  const data = res["data"];
  console.log(data);
}
//getPostTagList("life");
//getPostTagList("books");
...
async function getSearchTotal(product) {
  const res = await axios.get(`${BASE_URL}/products/search?q=${product}`);
  const data = res["data"];
